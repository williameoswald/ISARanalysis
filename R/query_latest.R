...
#' dates and returns the latest results within the search window.
#'
...
#'
#' An additional `{search_measure}_gap` variable is created with the calculated
#' interval in months between result date and search_end date.
#'
...
#' }
#'
#' @importFrom dplyr filter filter_out select distinct inner_join mutate bind_rows arrange slice_head
...
#'     search_measure = "spiro",
#'     search_anchor_date_col = "index_date",
#'     search_mos = 12,
#'     keep_df = TRUE #Return spiro_df dataset for reference
#'     ) |>
#'  dplyr::rename_with(.cols = -c("patient_id"), ~ paste0(., "_bl"))
#'
#' @export
query_latest <- function(
  df,
  query_dates,
  search_measure,
  search_anchor_date_col = "index_date",
